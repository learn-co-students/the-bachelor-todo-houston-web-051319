...
def get_contestant_name(data, occupation)
  data.each do |season, contestants|
    contestants.each do |contestant|
      if contestant[:occupation].downcase == occupation.downcase
...
def count_contestants_by_hometown(data, hometown)
  ...
  data.each do |season, contestants|
    contestants.each do |contestant|
      if contestant[:hometown].downcase == hometown.downcase
        counter += 1
      end
    end
  end
...
get_occupation(hash, "Waverly, Nebraska")
...
def get_average_age_for_season(data, season)
 ...
 sum = age_array.inject {|sum, num|sum + num}
 avg = sum / age_array.length
 avg.round
end
